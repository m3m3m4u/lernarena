...
export default function LueckentextPlayer({ lesson, courseId, completedLessons, setCompletedLessons, sessionUsername }: Props){
  ...
  const check=()=>{ const allCorrectNow= gaps.every(g=> (answersState[g.id]||'').trim()=== g.answer.trim()); let next={...answersState}; if(mode==='drag' && !allCorrectNow){ for(const g of gaps){ const val=(next[g.id]||'').trim(); if(val && val !== g.answer.trim()) next[g.id]=''; } setAnswersState(next); setUsedAnswers(Object.values(next).filter(Boolean)); } setChecked(true); setCorrectAll(allCorrectNow); if(allCorrectNow && !completedLessons.includes(lesson._id)){ (async()=>{ try{ if(sessionUsername){ await finalizeLesson({ username: sessionUsername, lessonId: lesson._id, courseId: lesson.courseId, type: lesson.type, earnedStar: lesson.type !== 'markdown' }); setCompletedLessons(prev=> prev.includes(lesson._id)? prev : [...prev, lesson._id]); } } catch{} })(); } };
