...
export default function OrderingPlayer({ lesson, courseId, completedLessons, setCompletedLessons, sessionUsername }: Props){
  ...
  const handleCheck=()=>{ const ok= current.every((v,i)=> v=== original[i]); setChecked(true); setCorrect(ok); if(ok && !completedLessons.includes(lesson._id)){ if(!sessionUsername) return; setSaving(true); (async()=>{ try{ await finalizeLesson({ username: sessionUsername, lessonId: lesson._id, courseId, type: lesson.type, earnedStar: true }); setCompletedLessons(prev=> prev.includes(lesson._id)? prev: [...prev, lesson._id]); } finally { setSaving(false);} })(); } };
